...
awful.key({modkey}, 'a', function()
    ...
    if s and s.left_panel then s.left_panel:toggle("drun") end
end, {description = 'show app menu', group = 'launcher'}),
awful.key({modkey}, 'Tab', function()
    local s = awful.screen.focused()
    if s and s.left_panel then s.left_panel:toggle("window") end
...
awful.key({modkey}, 'm', function() awful.layout.set(awful.layout.suit.max) end,
          {description = "use max layout", group = "layout"}), -- awful.key(
--  {altkey, 'Shift'},
--  'l',
--  function()
