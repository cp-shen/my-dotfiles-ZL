...
if screen.count() == 1 then
    tags = {
        settings = {
            {
                names = {
                    "1:Term", "2:Editor", "3:Browser", "4:Blender", "5:Godot",
                    "6:Mails", "7:Music", "8:Fun"
                },
    ...
    tags = {
        settings = {
            {
                names = {"1:Godot", "2:Mails", "3:Music", "4:Fun"},
                layout = {layouts[2], layouts[2], layouts[2], layouts[2]}
            }, {
                names = {"1:Term", "2:Editor", "3:Browser", "4:Blender"},
                layout = {layouts[2], layouts[2], layouts[2], layouts[2]}
            }
        }
    }
...
local mytextclock = wibox.widget.textclock(
                        markup(gray, " %a") .. markup(white, " %d ") ..
                            markup(gray, "%b ") .. markup(white, "%H:%M "))
...
theme.mpd = lain.widget.mpd({
    settings = function()
        ...
        widget:set_markup(markup.font(theme.font, markup(gray, artist) ..
                                          markup(white, title)))
    end
})
...
local bat = lain.widget.bat({
    settings = function()
        bat_header = " Bat "
        bat_p = bat_now.perc .. " "
        widget:set_markup(markup.font(theme.font, markup(gray, bat_header) ..
                                          markup(white, bat_p)))
    end
})
...
theme.volume = lain.widget.alsa({
    ...
    settings = function()
        ...
        widget:set_markup(markup.font(theme.font, markup(gray, header) ..
                                          markup(white, vlevel)))
    end
})
